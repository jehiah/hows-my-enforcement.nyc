...
curl -s "https://data.cityofnewyork.us/resource/${DATASET}.json?\$select=issuing_agency,issue_date,count(*)+as+number_violations&\$where=issuing_agency+in+(%27T%27,%27P%27,%27S%27)+AND+issue_date+is+not+null&\$group=issuing_agency,issue_date&\$having=number_violations+%3E+50&\$limit=5000" > temp.json || echo "error" >&2
